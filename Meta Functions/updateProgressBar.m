...
%     fprintf(full_bar(i))
% end
bar = [full_bar(1:filled_len), repmat(' ', 1, bar_len - filled_len)];
if ~isfield(d, 'shape')
    d.shape = "ideal";
end
...
fprintf("        Eb/N0 = %ddB, Fc = %dMHz, BW_channel = %.2fMHz , BW_subcar = %.2fkHz \n", ...
    d.EbN0, ...
    d.Fc / 1e6, ...
    BW, ...
    1 / (1000 * d.T));
...
fprintf("        T = %.2fus, t_{e,max} = %.1f*Ts, %s-shaped filters", ...
    d.T * 1e6, d.max_timing_offset, d.shape);
...
switch d.system_name
    case "OFDM"
        fprintf("        M_subcrs = %d, vel = %dkm/hr \n\n", ...
            d.M, d.vel);
    case "OTFS"
        fprintf("        M_subcrs = %d, N_tsyms = %d, vel = %dkm/hr \n\n", ...
            d.M, d.N, d.vel);
    case "ODDM"
        fprintf("        M_subcrs = %d, N_tsyms = %d, vel = %dkm/hr \n\n", ...
            d.M, d.N, d.vel);
    case "TODDM"
        fprintf("        M_subcrs = %d, N_tsyms = %d, U_frqlvls = %d, vel = %dkm/hr \n\n", ...
            d.M, d.N, d.U, d.vel);
end
