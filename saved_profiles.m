...
%% PROFILE 10
profile_name = "OTFS-DD MUSIC Estimation - N sweep w/ M";
p = struct;
p.primary_var = "M";
p.primary_vals = [32,64,128,256];
p.default_parameters = struct(...
    'channel_estimation_method', "MUSIC", ... % "none" assumes perfect CSI
    'system_name', "OTFS-DD",...
    'CP', true,...
    ...
    'alpha', 0.6, ...
    'Q', 2);
p.MUSIC_settings = struct(...
    'frames_per_trial', 20, ...
    'num_init_frames', 10, ...
    'num_pilot_frames_during_init', 10, ...
    'pilot_frame_frequency', 0, ...
    'num_pseudo_frames', 1, ...
    'num_pilots', 128, ...
    'R_x_size', 16, ...
    'use_true_x_cov', false, ...
    'enforce_toeplitz', false, ...
    'cov_epsilon', 0.0, ...
    'shrinkage_alpha', 0.0, ...
    'v_res', 1, ...
    'pilot_energy_alloc', 0.1);
p.vehicle_motion_settings = struct(...
    'simulate_vehicle', 0, ...
    'num_paths', 10, ...
    'bs_loc', 10, ...
    'multipath_range', 0, ...
    'min_bounce_dist', 1);
p.configs = {
    struct(N=16,num_pilots=16)
    struct(N=32,num_pilots=32)
    struct(N=64,num_pilots=64)
    };
p.delete_configs = [];
p.legend_vec = {
    "N = 16"
...
%% PROFILE 11
profile_name = "OTFS-DD MUSIC Estimation - Vel sweep w/ pulse shapes";
p = struct;
p.primary_var = "vel";
p.primary_vals = 100:100:600;
p.default_parameters = struct(...
    'channel_estimation_method', "MUSIC", ... % "none" assumes perfect CSI
    'system_name', "OTFS-DD",...
    'CP', true,...
...
p.vehicle_motion_settings = struct(...
    ...
    'min_bounce_dist', 1);
p.configs = {
    struct(M=32,shape="rect")
    struct(M=32,shape="sinc",Q=2)
    struct(M=32,shape="rrc",Q=2,alpha=0.6)
    struct(M=64,shape="rect")
    struct(M=64,shape="sinc",Q=2)
    struct(M=64,shape="rrc",Q=2,alpha=0.6)
    };
p.delete_configs = [];
p.legend_vec = {
    "M = 32, rect"
