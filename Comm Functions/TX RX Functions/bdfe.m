...
for k = N_tx_sym:-1:1
   ...
   dec_vec = dec_vec-abs(temp_value-Feedback_mat(k, k)*syms.').^2*D(k, k);
   % find the LL of '0' and '1'
   for m = 1:N_bit_per_sym
		d0_mat(m, :) = d0_mat(m, :)-abs(temp_value-Feedback_mat(k, k)*group0(m, :)).^2*D(k, k);
		d1_mat(m, :) = d1_mat(m, :)-abs(temp_value-Feedback_mat(k, k)*group1(m, :)).^2*D(k, k);
    end
