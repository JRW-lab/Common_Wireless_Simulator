...
% Find all possible Lambda_n and Theta_n matrices, and the MMSE matrix
% W_n they produce. W_n depends only on Lambda_n, Theta_n, N0 and Es -
% none of which change across equalizer iterations - so it is computed
% once here per block n and reused every iteration, instead of being
% recomputed on every pass below.
...
for n = 0:M-1
    ...
    possible_W_n((n*N)+1:(n+1)*N,:) = Lambda_n' / (Lambda_n*Lambda_n' + (N0/Es)*Theta_n);
end
